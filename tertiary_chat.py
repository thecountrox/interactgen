...
def detect_page_complexity(html: str) -> Dict[str, Any]:
    ...
    try:
        ...
        technical_keywords = {
            'beginner': ['tutorial', 'introduction', 'getting started', 'basics', 'learn', 'guide'],
            'intermediate': ['configuration', 'setup', 'api', 'integration', 'documentation'],
            'advanced': ['optimization', 'performance', 'architecture', 'advanced', 'algorithm', 
                        'scaling', 'distributed', 'concurrent', 'async'],
            'expert': ['internals', 'low-level', 'compiler', 'kernel', 'protocol', 'specification',
                      'rfc', 'implementation details', 'source code']
        }
        
        # Programming concepts
        programming_concepts = {
            'react': ['react', 'jsx', 'component', 'hook', 'useeffect', 'usestate', 'props'],
            'backend': ['api', 'database', 'server', 'endpoint', 'rest', 'graphql', 'sql'],
            'devops': ['docker', 'kubernetes', 'deployment', 'ci/cd', 'pipeline', 'container'],
            'security': ['authentication', 'authorization', 'encryption', 'oauth', 'jwt', 'security'],
            'data': ['machine learning', 'ai', 'neural network', 'model', 'training', 'dataset']
        }
        
        # Determine complexity level
        complexity_scores = {}
        for level, keywords in technical_keywords.items():
            score = sum(1 for keyword in keywords if keyword in text or keyword in title)
...
async def generate_proactive_nudge(
    ...
    try:
        ...
        logger.info(f"📊 Page analysis: {page_level} level, concepts: {concepts}, type: {page_type}")
        logger.info(f"👤 User level: {user_level}")
        
        # Check for knowledge gap
        level_hierarchy = ['beginner', 'intermediate', 'advanced', 'expert']
        user_level_idx = level_hierarchy.index(user_level) if user_level in level_hierarchy else 1
        page_level_idx = level_hierarchy.index(page_level) if page_level in level_hierarchy else 1
...
async def generate_nudge_with_llm(
    ...
    instruction = nudge_instructions.get(nudge_type, "Provide a helpful tip about this page.")
...
def generate_fallback_nudge(
    ...
    return templates.get(nudge_type, f"👋 {username}, I'm here to help if you need anything!")
