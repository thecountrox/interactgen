...
async def process_memory_background(
    ...
    """
    try:
        logger.info(f"🧠 Processing memory for user {user_id}: {interaction_summary[:100]}...")
...
async def process_multiple_memories(
    ...
    logger.info(f"Batch processing complete: {results['success_count']} success, {results['failed_count']} failed")
    return results
...
async def cleanup_old_memories(
    ...
    try:
        ...
        # Delete old memories
        result = supabase.table("memories").delete().eq("user_id", user_id).lt("created_at", cutoff_date).execute()
...
async def create_test_memory(
    ...
    scenarios = {
        ...
        "form": {
            ...
            "metadata": {"action": "form_submission", "url": "https://example.com/contact"}
        },
        "search": {
